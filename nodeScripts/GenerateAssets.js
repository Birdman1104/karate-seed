...
async function generateAtlas(data, name) {
    ...
    texturePacker(assets, options, async (files, error) => {
        ...
        for (const item of files) {
            const itemPath = join(assetsPath, `atlas/${item.name}`);
...
async function generateAtlases() {
    ...
    try {
        ...
        const data = `export const atlases: string[] = ${JSON.stringify(atlasNames)}`;
        const file = join(assetsPath, "assetsNames/atlases.ts");
